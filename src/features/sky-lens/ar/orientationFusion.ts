...
/**
 * A magnetic reading disagreeing by more than this while the phone is not turning is an
 * outlier (a passing magnet, a steel door frame, a sudden field reversal) and is discarded.
 */
export const HEADING_OUTLIER_DEGREES = 22;
...
export interface HeadingCorrectionResult {
  ...
  reason: "applied" | "frozen" | "ill-conditioned" | "outlier";
...
export function correctHeading(input: HeadingCorrectionInput): HeadingCorrectionResult {
  ...
  //    drift may neither move the view nor unlock it.
  ...
  // 3. Outlier rejection. A large disagreement while barely turning is magnetic, not real.
  const delta = shortestAngleDelta(currentHeading, measuredHeading);
  if (Math.abs(delta) > HEADING_OUTLIER_DEGREES && gyroSpeed < QUIET_GYRO_RAD_PER_SEC) {
    return unchanged("outlier");
  }

  // 4. Bounded nudge: a small fraction of the disagreement, hard-capped per sample, along
  //    the shortest path so 359 -> 1 travels 2 degrees rather than 358.
  ...
  return { heading: normalizeHeading(currentHeading + applied), reason: "applied", appliedDegrees: applied };
}
