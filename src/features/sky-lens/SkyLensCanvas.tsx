...
type Props = {
  ...
  /** During a physical pan, draw a lighter sky and restore full detail after settling. */
...
export function SkyLensCanvas({
  ...
  const domeStars = useMemo(() => {
    const base = domeStarMultiplier < 1
      ...
      : sky.domeStars;
    const limit = reducedDetail ? 3.25 : 4.5;
    return base.filter((s) => s.aboveHorizon && s.magnitude <= limit);
  }, [sky.domeStars, domeStarMultiplier, reducedDetail]);

  const primaryStars = useMemo(
    () => reducedDetail ? sky.stars.filter((star) => star.magnitude <= 2.7) : sky.stars,
    [sky.stars, reducedDetail]
  );

  const showLabels = !cinematic && !reducedDetail;
  ...
  return (
    <Svg style={StyleSheet.absoluteFillObject} pointerEvents="box-none">
      ...
      {moonOnScreen && !nightMode && moonProj && !reducedDetail && (
      ...
      <G opacity={heroDim}>
        ...
        {!cinematic && !reducedDetail && <CardinalLayer project={project} box={box} nightMode={nightMode} />}
        ...
        {activeLayers.has("constellations") && (
          <G opacity={cinematic ? 0.48 : reducedDetail ? 0.5 : 0.72}>
            <ConstellationLayer
        ...
        {activeLayers.has("stars") && (
          <G transform={depth(0.25)}>
            <StarLayer
              ...
              extinction={extinction}
              bloom={vg.starBloom && !reducedDetail}
              fullSphere={horizonCorrect}
              onSelect={onSelect}
            />
          </G>
        ...
        {vg.shootingStars && !reducedDetail && (
        ...
        {activeLayers.has("satellites") && !cinematic && !reducedDetail && (
          <SatelliteLayer
